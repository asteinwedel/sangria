        ...
        (typeInfo, deprecated) =>
          AstVisitor.simple {
              ...
              val (key, deprecation) =
                (typeInfo.directive, typeInfo.previousParentType, typeInfo.fieldDef) match {
                  case (Some(directive), _, _) =>
                    ...
                    val deprecation = DeprecatedDirectiveArgument(directive, arg, deprecationReason)

                    (key, deprecation)
                  case (_, Some(parent), Some(field)) =>
                    val key = parent.name + "." + field.name + "." + arg.name
                    val deprecation = DeprecatedFieldArgument(parent, field, arg, deprecationReason)

                    (key, deprecation)
                }

              if (!deprecated.contains(key))
                deprecated(key) = deprecation
            ...
            case _: ast.ObjectField
              ...
              typeInfo.argument.map(_.argumentType) match {
